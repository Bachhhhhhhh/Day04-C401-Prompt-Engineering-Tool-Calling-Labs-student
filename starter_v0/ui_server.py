...
def tool_results_message(events: list[dict[str, Any]]) -> dict[str, str]:
    return {
        ...
        "content": (
            ...
            "Use only these tool results. Answer the user directly in Vietnamese when possible. "
            "If the user asked for a digest and the items are ready, call the formatting tool."
        ),
    }
...
def run_model_tool_loop(
    *,
    ...
    return {
        ...
        "assistant_text": f"Đã dừng sau {max_tool_rounds} vòng gọi tool. Hãy xem log công cụ để biết chi tiết.",
        "rounds": rounds,
        "tool_events": all_tool_events,
    }
